...
const posts = [
  new Post({ name: 'Prashant', title: 'Hello Mern', visits: '0', slug: 'hello-mern', cuid: 'f34gb2bh24b24b2', content: "All cats meow 'mern!'" }),
  new Post({ name: 'Mayank', title: 'Hi Mern', slug: 'hi-mern', visits: '0', cuid: 'f34gb2bh24b24b3', content: "All dogs bark 'mern!'" }),
];
...
test.serial('Should send correct data when queried against a cuid', async t => {
  ...
  const post = new Post({ name: 'Foo', title: 'bar', slug: 'bar', visits: '0', cuid: 'f34gb2bh24b24b2', content: 'Hello Mern says Foo' });
  post.save();
...
test.serial('Should correctly add a post', async t => {
  ...
  const res = await request(app)
    .post('/api/posts')
    .send({ post: { name: 'Foo', title: 'bar', visits: '0', content: 'Hello Mern says Foo' } })
    .set('Accept', 'application/json');
...
test.serial('Should correctly delete a post', async t => {
  ...
  const post = new Post({ name: 'Foo', title: 'bar', visits: '0', slug: 'bar', cuid: 'f34gb2bh24b24b2', content: 'Hello Mern says Foo' });
  post.save();
